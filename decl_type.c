...
visit_parser_state(Ast* ast)
{
  ...
  Ast* stmt_list = state_decl->stmt_list;
  if (stmt_list) {
    DList* li = stmt_list->children.members.next;
    while (li) {
      Ast* stmt = li->object;
...
visit_struct(Ast* ast)
{
  ...
  if (fields) {
    DList* field_li = fields->children.members.next;
    while (field_li) {
...
visit_function(Ast* ast)
{
  ...
  if (function_body) {
    Ast* stmt_list = function_body->stmt_list;
    if (stmt_list) {
      DList* li = stmt_list->children.members.next;
      while (li) {
        Ast* stmt = li->object;
...
visit_action(Ast* ast)
{
  ...
  if (params) {
    DList* param_li = params->children.members.next;
    while (param_li) {
  ...
  if (action_body) {
    ...
    if (stmt_list) {
      DList* stmt_li = stmt_list->children.members.next;
      while (stmt_li) {
...
visit_p4program(Ast* ast)
{
  ...
  if (decl_list) {
    DList* decl_li = decl_list->children.members.next;
    while (decl_li) {
