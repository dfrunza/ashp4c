...
internal void
traverse_ast(Ast* ast, AstVisit* visit)
{
  visit(ast, WALK_IN);
  ...
  if (ast->kind == AST_p4program) {
    ...
    traverse_ast(param->name, visit);
    traverse_ast(param->type, visit);
    if (param->init_expr) {
      traverse_ast(param->init_expr, visit);
    }
  } else if (ast->kind == AST_packageTypeDeclaration) {
    Ast_PackageTypeDeclaration* package_decl = (Ast_PackageTypeDeclaration*)ast;
    traverse_ast(package_decl->name, visit);
    if (package_decl->type_params) {
      traverse_ast(package_decl->type_params, visit);
    }
    if (package_decl->params) {
      traverse_ast(package_decl->params, visit);
    }
  } else if (ast->kind == AST_instantiation) {
    Ast_Instantiation* inst_decl = (Ast_Instantiation*)ast;
    traverse_ast(inst_decl->type_ref, visit);
    if (inst_decl->args) {
      traverse_ast(inst_decl->args, visit);
    }
    traverse_ast(inst_decl->name, visit);
  }

  /** PARSER **/
  else if (ast->kind == AST_parserDeclaration) {
    Ast_ParserDeclaration* parser_decl = (Ast_ParserDeclaration*)ast;
    traverse_ast(parser_decl->proto, visit);
    if (parser_decl->ctor_params) {
      traverse_ast(parser_decl->ctor_params, visit);
    }
    if (parser_decl->local_elements) {
      traverse_ast(parser_decl->local_elements, visit);
    }
    if (parser_decl->states) {
      traverse_ast(parser_decl->states, visit);
    }
  } else if (ast->kind == AST_parserTypeDeclaration) {
    Ast_ParserPrototype* parser_proto = (Ast_ParserPrototype*)ast;
    traverse_ast(parser_proto->name, visit);
    if (parser_proto->type_params) {
      traverse_ast(parser_proto->type_params, visit);
    }
    if (parser_proto->params) {
      traverse_ast(parser_proto->params, visit);
    }
    ...
    traverse_ast(local_elem->element, visit);
    ...
    traverse_ast(parser_state->name, visit);
    if (parser_state->stmt_list) {
      traverse_ast(parser_state->stmt_list, visit);
    }
    if (parser_state->transition_stmt) {
      traverse_ast(parser_state->transition_stmt, visit);
    }
    ...
    traverse_ast(select_case->keyset_expr, visit);
    traverse_ast(select_case->name, visit);
  } else if (ast->kind == AST_keysetExpression) {
    Ast_KeysetExpression* keyset_expr = (Ast_KeysetExpression*)ast;
    traverse_ast(keyset_expr->expr, visit);
    ...
    traverse_ast(keyset_expr->expr_list, visit);
  ...
  /** CONTROL **/
    ...
    traverse_ast(control_decl->proto, visit);
    if (control_decl->ctor_params) {
      traverse_ast(control_decl->ctor_params, visit);
    }
    if (control_decl->local_decls) {
      traverse_ast(control_decl->local_decls, visit);
    }
    if (control_decl->apply_stmt) {
      traverse_ast(control_decl->apply_stmt, visit);
    }
  } else if (ast->kind == AST_controlTypeDeclaration) {
    Ast_ControlPrototype* control_proto = (Ast_ControlPrototype*)ast;
    traverse_ast(control_proto->name, visit);
    if (control_proto->type_params) {
      traverse_ast(control_proto->type_params, visit);
    }
    if (control_proto->params) {
      traverse_ast(control_proto->params, visit);
    }
    ...
    traverse_ast(local_decl->decl, visit);
  }

  /** EXTERN **/
  else if (ast->kind == AST_externDeclaration) {
    Ast_ExternDeclaration* extern_decl = (Ast_ExternDeclaration*)ast;
    traverse_ast(extern_decl->decl, visit);
  } else if (ast->kind == AST_externType) {
    Ast_ExternType* extern_type = (Ast_ExternType*)ast;
    traverse_ast(extern_type->name, visit);
    traverse_ast(extern_type->type_params, visit);
    traverse_ast(extern_type->method_protos, visit);
    ...
    if (func_proto->return_type) {
      traverse_ast(func_proto->return_type, visit);
    }
    traverse_ast(func_proto->name, visit);
    if (func_proto->type_params) {
      traverse_ast(func_proto->type_params, visit);
    }
    if (func_proto->params) {
      traverse_ast(func_proto->params, visit);
    }
  }
  ...
  /** TYPES **/
    ...
    traverse_ast(type_arg->arg, visit);
    ...
    traverse_ast(type_decl->decl, visit);
    ...
    if (match_decl->fields) {
      traverse_ast(match_decl->fields, visit);
    }
  ...
  /** STATEMENTS **/
    ...
    traverse_ast(if_stmt->cond_expr, visit);
    if (if_stmt->else_stmt) {
      traverse_ast(if_stmt->else_stmt, visit);
    }
  } else if (ast->kind == AST_directApplication) {
    Ast_DirectApplication* apply_stmt = (Ast_DirectApplication*)ast;
    traverse_ast(apply_stmt->lhs_expr, visit);
    traverse_ast(apply_stmt->args, visit);
  } else if (ast->kind == AST_statement) {
    Ast_Statement* stmt = (Ast_Statement*)ast;
    traverse_ast(stmt->stmt, visit);
    ...
    traverse_ast(switch_stmt->expr, visit);
    if (switch_stmt->switch_cases) {
      traverse_ast(switch_stmt->switch_cases, visit);
    }
    ...
    traverse_ast(switch_case->label, visit);
    if (switch_case->stmt) {
      traverse_ast(switch_case->stmt, visit);
    }
  }
  else if (ast->kind == AST_switchLabel) {
    Ast_SwitchLabel* switch_label = (Ast_SwitchLabel*)ast;
    traverse_ast(switch_label->label, visit);
  ...
  /** TABLES **/
    ...
    traverse_ast(table_decl->name, visit);
    if (table_decl->prop_list) {
      traverse_ast(table_decl->prop_list, visit);
    }
    ...
    traverse_ast(keyelem->expr, visit);
    traverse_ast(keyelem->name, visit);
    ...
    traverse_ast(action_ref->name, visit);
    if (action_ref->args) {
      traverse_ast(action_ref->args, visit);
    }
    ...
    traverse_ast(entry->keyset, visit);
    traverse_ast(entry->action, visit);
  } else if (ast->kind == AST_actionDeclaration) {
    Ast_ActionDeclaration* action_decl = (Ast_ActionDeclaration*)ast;
    traverse_ast(action_decl->name, visit);
    if (action_decl->params) {
      traverse_ast(action_decl->params, visit);
    }
    if (action_decl->stmt) {
      traverse_ast(action_decl->stmt, visit);
    }
  }
  ...
  /** VARIABLES **/
    ...
    traverse_ast(var_decl->name, visit);
    traverse_ast(var_decl->type, visit);
    if (var_decl->init_expr) {
      traverse_ast(var_decl->init_expr, visit);
    }
  } else if (ast->kind == AST_constantDeclaration) {
    Ast_ConstDeclaration* const_decl = (Ast_ConstDeclaration*)ast;
    traverse_ast(const_decl->type, visit);
    traverse_ast(const_decl->name, visit);
    traverse_ast(const_decl->init_expr, visit);
  }

  /** EXPRESSIONS **/
  else if (ast->kind == AST_functionDeclaration) {
    Ast_FunctionDeclaration* func_decl = (Ast_FunctionDeclaration*)ast;
    traverse_ast(func_decl->proto, visit);
    if (func_decl->stmt) {
      traverse_ast(func_decl->stmt, visit);
    }
...
void traverse_p4program(Ast_P4Program* p4program, AstVisit* visit)
{
  traverse_ast((Ast*)p4program, visit);
}
