...
internal void
traverse_ast(Ast* ast, Ast* parent_ast, AstVisitor visitor)
{
  visitor(ast, parent_ast, WALK_IN);
  if (ast->kind == AST_p4program) {
    ...
    traverse_ast(member_expr->lhs_expr, ast, visitor);
    traverse_ast(member_expr->member_name, ast, visitor);
    ...
    traverse_ast(cast_expr->to_type, ast, visitor);
    traverse_ast(cast_expr->expr, ast, visitor);
  } else if (ast->kind == AST_arraySubscript) {
    Ast_ArraySubscript* subscript_expr = (Ast_ArraySubscript*)ast;
    traverse_ast(subscript_expr->index, ast, visitor);
    if (subscript_expr->end_index) {
      traverse_ast(subscript_expr->end_index, ast, visitor);
    }
  } else if (ast->kind == AST_kvPairExpression) {
    Ast_KVPairExpr* kv_expr = (Ast_KVPairExpr*)ast;
    traverse_ast(kv_expr->name, ast, visitor);
    traverse_ast(kv_expr->expr, ast, visitor);
    ...
    if (table_prop->init_expr) {
      traverse_ast(table_prop->init_expr, ast, visitor);
    }
    ...
    traverse_ast(entry->keyset, ast, visitor);
    traverse_ast(entry->action, ast, visitor);
    ...
    traverse_ast(keyelem->expr, ast, visitor);
    traverse_ast(keyelem->name, ast, visitor);
  } else if (ast->kind == AST_actionRef) {
    Ast_ActionRef* action_ref = (Ast_ActionRef*)ast;
    traverse_ast(action_ref->name, ast, visitor);
    ...
    traverse_ast(state->trans_stmt, ast, visitor);
    ...
    traverse_ast(select_case->keyset, ast, visitor);
    traverse_ast(select_case->name, ast, visitor);
  } else if (ast->kind == AST_variableDeclaration) {
    Ast_Var* var_decl = (Ast_Var*)ast;
    traverse_ast(var_decl->type, ast, visitor);
    if (var_decl->init_expr) {
      traverse_ast(var_decl->init_expr, ast, visitor);
    }
    ...
    traverse_ast(if_stmt->cond_expr, ast, visitor);
    if (if_stmt->else_stmt) {
      traverse_ast(if_stmt->else_stmt, ast, visitor);
    }
  } else if (ast->kind == AST_switchStatement) {
    Ast_SwitchStmt* switch_stmt = (Ast_SwitchStmt*)ast;
    traverse_ast(switch_stmt->expr, ast, visitor);
    ...
    traverse_ast(assign_stmt->lvalue, ast, visitor);
    traverse_ast(assign_stmt->expr, ast, visitor);
  } else if (ast->kind == AST_returnStatement) {
    Ast_ReturnStmt* return_stmt = (Ast_ReturnStmt*)ast;
    if (return_stmt->expr) {
      traverse_ast(return_stmt->expr, ast, visitor);
    }
    ...
    traverse_ast(id->name, ast, visitor);
    if (id->init_expr) {
      traverse_ast(id->init_expr, ast, visitor);
    }
    ...
    if (ctrl_decl->apply_stmt) {
      traverse_ast(ctrl_decl->apply_stmt, ast, visitor);
    }
    ...
    traverse_ast(inst_decl->type, ast, visitor);
    ...
    traverse_ast(type_decl->type_ref, ast, visitor);
    ...
    if (func_proto->return_type) {
      traverse_ast(func_proto->return_type, ast, visitor);
    }
    ...
    if (func_decl->stmt) {
      traverse_ast(func_decl->stmt, ast, visitor);
    ...
    traverse_ast(const_decl->type, ast, visitor);
    traverse_ast(const_decl->expr, ast, visitor);
    ...
    if (action_decl->stmt) {
      traverse_ast(action_decl->stmt, ast, visitor);
  ...
  visitor(ast, parent_ast, WALK_OUT);
}

void traverse_p4program(Ast_P4Program* p4program, AstVisitor visitor)
{
  traverse_ast((Ast*)p4program, 0, visitor);
}
