...
visit_parameter(Ast_Parameter* param)
{
  ...
  declslot_push_decl(name_storage, &current_scope->decls, namedecl, NS_VAR);
  if (param->init_expr) {
...
visit_identifierList(Ast_IdentifierList* ident_list, Hashmap* name_table)
{
  ...
  for (ListItem* li = list_first_item(&ident_list->members);
    ...
    namedecl->line_no = name->line_no;
    namedecl->column_no = name->column_no;
    declslot_push_decl(name_storage, name_table, namedecl, NS_VAR);
  }
}
...
name_decl_pass(Ast_P4Program* p4program, Arena* name_storage_)
{
  ...
  NameDecl* namedecl;
  namedecl = arena_push_struct(name_storage, NameDecl);
