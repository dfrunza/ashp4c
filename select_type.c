...
visit_param(Ast* ast)
{
  ...
  /*
  Type_TypeSet* ty_set = typeset_get(possible_type, param->id);
...
visit_control_proto(Ast* ast)
{
  ...
  /*
  Type_TypeSet* ty_set = typeset_get(possible_type, ctrl_proto->id);
  ...
  proto_ty->return_ty = typeset_get(possible_type, void_decl->id)->members.next->object;
  */
  DList* li;
...
visit_package(Ast* ast)
{
  ...
  /*
  Type_TypeSet* ty_set = typeset_get(possible_type, package_decl->id);
...
visit_function_proto(Ast* ast)
{
  ...
  /*
  Type_TypeSet* ty_set = typeset_get(possible_type, func_proto->id);
  Type_Function* proto_ty = ty_set->members.next->object;
...
Hashmap*
select_type(Ast_P4Program* p4program, Scope* root_scope_, Hashmap* possible_type_, Arena* type_storage_)
{
  root_scope = root_scope_;
  possible_type = possible_type_;
