...
static void visit_arraySubscript(Ast* subscript, Type* required_ty);
static void visit_indexExpression(Ast* index_expr, Type* required_ty);
static void visit_booleanLiteral(Ast* bool_literal, Type* required_ty);
static void visit_integerLiteral(Ast* int_literal, Type* required_ty);
static void visit_stringLiteral(Ast* str_literal, Type* required_ty);
...
visit_lvalueExpression(Ast* lvalue_expr, Type* required_ty)
{
  ...
  if (lvalue_expr->lvalueExpression.expr->kind == AST_name) {
    ...
    visit_arraySubscript(lvalue_expr->lvalueExpression.expr, required_ty);
  } else assert(0);
...
visit_expression(Ast* expr, Type* required_ty)
{
  ...
  if (expr->expression.expr->kind == AST_expression) {
    ...
    visit_booleanLiteral(expr->expression.expr, required_ty);
  } else if (expr->expression.expr->kind == AST_integerLiteral) {
    visit_integerLiteral(expr->expression.expr, required_ty);
  } else if (expr->expression.expr->kind == AST_stringLiteral) {
    visit_stringLiteral(expr->expression.expr, required_ty);
    ...
    visit_arraySubscript(expr->expression.expr, required_ty);
...
static void
visit_arraySubscript(Ast* subscript, Type* required_ty)
{
  assert(subscript->kind == AST_arraySubscript);
  if (subscript->arraySubscript.lhs_expr->kind == AST_expression) {
    visit_expression(subscript->arraySubscript.lhs_expr, required_ty);
  } else if (subscript->arraySubscript.lhs_expr->kind == AST_lvalueExpression) {
    visit_lvalueExpression(subscript->arraySubscript.lhs_expr, required_ty);
  } else assert(0);
  visit_indexExpression(subscript->arraySubscript.index_expr, required_ty);
}

static void
visit_indexExpression(Ast* index_expr, Type* required_ty)
{
  assert(index_expr->kind == AST_indexExpression);
  visit_expression(index_expr->indexExpression.start_index, required_ty);
  if (index_expr->indexExpression.end_index) {
    visit_expression(index_expr->indexExpression.end_index, required_ty);
  }
}

static void
visit_booleanLiteral(Ast* bool_literal, Type* required_ty)
{
  assert(bool_literal->kind == AST_booleanLiteral);
}

static void
visit_integerLiteral(Ast* int_literal, Type* required_ty)
{
...
static void
visit_stringLiteral(Ast* str_literal, Type* required_ty)
{
