...
visit_name(Ast_Name* name)
{
  ...
  if (hashmap_lookup(type_table, HASHMAP_KEY_STRING, name->strname) == 0) {
    ...
    hashmap_set(type_table, HASHMAP_KEY_STRING, name->strname, &name_ty);
  }
}
...
visit_packageTypeDeclaration(Ast_PackageTypeDeclaration* package_decl)
{
  ...
  hashmap_set(type_table, HASHMAP_KEY_STRING, name->strname, &package_ty);
...
visit_parserTypeDeclaration(Ast_ParserTypeDeclaration* parser_decl)
{
  ...
  hashmap_set(type_table, HASHMAP_KEY_STRING, name->strname, &parser_ty);
...
visit_controlTypeDeclaration(Ast_ControlTypeDeclaration* control_decl)
{
  ...
  hashmap_set(type_table, HASHMAP_KEY_STRING, name->strname, &control_ty);
...
visit_functionPrototype(Ast_FunctionPrototype* func_proto)
{
  ...
  hashmap_set(type_table, HASHMAP_KEY_STRING, name->strname, &func_proto);
...
visit_tupleType(Ast_TupleType* type_decl)
{
  ...
  for (Ast** p = list_cursor_begin(&type_args->members);
       p != 0; p = list_cursor_next(&type_args->members)) {
    Ast_TypeArg* type_arg = (Ast_TypeArg*)(*p);
...
visit_headerStackType(Ast_HeaderStackType* type_decl)
{
  ...
  hashmap_set(type_table, HASHMAP_KEY_STRING, name->strname, &stack_ty);
...
visit_specializedType(Ast_SpecializedType* type_decl)
{
  ...
  hashmap_set(type_table, HASHMAP_KEY_STRING, name->strname, &speclzd_ty);
  Ast_TypeRef* type = (Ast_TypeRef*)type_decl->type;
  visit_typeRef(type);
...
visit_headerTypeDeclaration(Ast_HeaderTypeDeclaration* header_decl)
{
  ...
  for (Ast** p = list_cursor_begin(&fields->members);
       p != 0; p = list_cursor_next(&fields->members)) {
    Ast_StructField* field = (Ast_StructField*)(*p);
...
visit_enumDeclaration(Ast_EnumDeclaration* enum_decl)
{
  ...
  hashmap_set(type_table, HASHMAP_KEY_STRING, name->strname, &enum_ty);
...
pass_type_decl(ParsedProgram* p4program, Arena* _storage)
{
  ...
  for (int i = 0; i < sizeof(basic_types)/sizeof(basic_types[0]); i++) {
    ...
    hashmap_set(type_table, HASHMAP_KEY_STRING, basic_ty->strname, &basic_ty);
  }

  Type_TypeVar* dontcare_ty = arena_malloc(storage, sizeof(*dontcare_ty));
  dontcare_ty->ctor = TYPE_TYPEVAR;
  dontcare_ty->strname = "_";
  hashmap_set(type_table, HASHMAP_KEY_STRING, dontcare_ty->strname, &dontcare_ty);
