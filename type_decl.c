...
visit_name(Ast_Name* name)
{
  ...
  if (!hashmap_lookup(&type_table, HKEY_STRING, name->strname)) {
    ...
    hashmap_set(&type_table, storage, &name_ty, HKEY_STRING, name->strname);
  }
}
...
visit_packageTypeDeclaration(Ast_PackageTypeDeclaration* package_decl)
{
  ...
  hashmap_set(&type_table, storage, &package_ty, HKEY_STRING, name->strname);
...
visit_parserTypeDeclaration(Ast_ParserTypeDeclaration* parser_decl)
{
  ...
  hashmap_set(&type_table, storage, &parser_ty, HKEY_STRING, name->strname);
...
visit_controlTypeDeclaration(Ast_ControlTypeDeclaration* control_decl)
{
  ...
  hashmap_set(&type_table, storage, &control_ty, HKEY_STRING, name->strname);
...
visit_externTypeDeclaration(Ast_ExternTypeDeclaration* extern_decl)
{
  ...
  hashmap_set(&type_table, storage, &extern_ty, HKEY_STRING, name->strname);
...
visit_functionPrototype(Ast_FunctionPrototype* func_proto)
{
  ...
  hashmap_set(&type_table, storage, &func_proto, HKEY_STRING, name->strname);
...
visit_tupleType(Ast_TupleType* type_decl)
{
  ...
  hashmap_set(&type_table, storage, &tuple_ty, HKEY_STRING, name->strname);
...
visit_headerStackType(Ast_HeaderStackType* type_decl)
{
  ...
  hashmap_set(&type_table, storage, &stack_ty, HKEY_STRING, name->strname);
...
visit_specializedType(Ast_SpecializedType* type_decl)
{
  ...
  hashmap_set(&type_table, storage, &speclzd_ty, HKEY_STRING, name->strname);
  Ast_TypeRef* type = (Ast_TypeRef*)type_decl->type;
  visit_typeRef(type);
...
visit_headerTypeDeclaration(Ast_HeaderTypeDeclaration* header_decl)
{
  ...
  hashmap_set(&type_table, storage, &header_ty, HKEY_STRING, name->strname);
...
visit_headerUnionDeclaration(Ast_HeaderUnionDeclaration* union_decl)
{
  ...
  hashmap_set(&type_table, storage, &union_ty, HKEY_STRING, name->strname);
...
visit_structTypeDeclaration(Ast_StructTypeDeclaration* struct_decl)
{
  ...
  hashmap_set(&type_table, storage, &struct_ty, HKEY_STRING, name->strname);
...
visit_enumDeclaration(Ast_EnumDeclaration* enum_decl)
{
  ...
  hashmap_set(&type_table, storage, &enum_ty, HKEY_STRING, name->strname);
...
visit_typedefDeclaration(Ast_TypedefDeclaration* typedef_decl)
{
  ...
  hashmap_set(&type_table, storage, &typedef_ty, HKEY_STRING, name->strname);
...
visit_actionDeclaration(Ast_ActionDeclaration* action_decl)
{
  ...
  hashmap_set(&type_table, storage, &action_decl, HKEY_STRING, name->strname);
...
{
  storage = _storage;
  hashmap_create(&type_table, storage, HKEY_STRING, sizeof(Type*), 15, 1023);
  ...
  for (int i = 0; i < sizeof(basic_types)/sizeof(basic_types[0]); i++) {
    ...
    hashmap_set(&type_table, storage, &basic_ty, HKEY_STRING, basic_ty->strname);
  }

  Type_TypeVar* dontcare_ty = arena_malloc(storage, sizeof(*dontcare_ty));
  dontcare_ty->ctor = TYPE_TYPEVAR;
  dontcare_ty->strname = "_";
  hashmap_set(&type_table, storage, &dontcare_ty, HKEY_STRING, dontcare_ty->strname);
