...
visit_p4program(Ast_P4Program* p4program)
{
  ...
  hashmap_create(&scope->name_table, storage, HASHMAP_KEY_STRING, sizeof(NameEntry), 7, 511);
...
visit_name(Ast_Name* name)
{
  ...
  hashmap_set(scope_map, HASHMAP_KEY_UINT32, (uint64_t)name, &current_scope);
}
...
visit_parserStatements(Ast_ParserStatements* stmts)
{
  ...
  for (Ast* ast = list_cursor_begin(&stmts->members);
...
visit_methodPrototypes(Ast_MethodPrototypes* protos)
{
  ...
  for (Ast* ast = *(Ast**)list_cursor_begin(&protos->members);
       ast != 0; ast = *(Ast**)list_cursor_next(&protos->members)) {
...
visit_typeParameterList(Ast_TypeParameterList* param_list)
{
  ...
  for (Ast_Name* name = *(Ast_Name**)list_cursor_begin(&param_list->members);
       name != 0; name = *(Ast_Name**)list_cursor_next(&param_list->members)) {
    NameEntry* ns = scope_lookup_any(current_scope, name->strname);
    if (ns && ns->ns[NS_TYPE]) {
      visit_name(name);
    } else {
      NameDecl* namedecl = arena_malloc(storage, sizeof(*namedecl));
...
visit_headerTypeDeclaration(Ast_HeaderTypeDeclaration* header_decl)
{
  ...
  hashmap_set(field_map, HASHMAP_KEY_UINT32, (uint64_t)header_decl, &field_scope);
}
...
visit_headerUnionDeclaration(Ast_HeaderUnionDeclaration* union_decl)
{
  ...
  hashmap_set(field_map, HASHMAP_KEY_UINT32, (uint64_t)union_decl, &field_scope);
}
...
visit_structTypeDeclaration(Ast_StructTypeDeclaration* struct_decl)
{
  ...
  hashmap_set(field_map, HASHMAP_KEY_UINT32, (uint64_t)struct_decl, &field_scope);
}
...
visit_enumDeclaration(Ast_EnumDeclaration* enum_decl)
{
  ...
  hashmap_set(field_map, HASHMAP_KEY_UINT32, (uint64_t)enum_decl, &field_scope);
}
...
visit_errorDeclaration(Ast_ErrorDeclaration* error_decl)
{
  ...
  hashmap_set(field_map, HASHMAP_KEY_UINT32, (uint64_t)error_decl, &field_scope);
}
...
visit_matchKindDeclaration(Ast_MatchKindDeclaration* match_decl)
{
  ...
  hashmap_set(field_map, HASHMAP_KEY_UINT32, (uint64_t)match_decl, &field_scope);
}
...
pass_name_decl(ParsedProgram* p4program, Arena* _storage)
{
  ...
  hashmap_create(scope_map, storage, HASHMAP_KEY_UINT32, sizeof(Scope*), 7, 1023);
  field_map = &pass_result.field_map;
  hashmap_create(field_map, storage, HASHMAP_KEY_UINT32, sizeof(Scope*), 7, 1023);
