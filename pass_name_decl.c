...
visit_p4program(Ast_P4Program* p4program)
{
  ...
  hashmap_create(&scope->decls, storage, HASHMAP_KEY_STRING, 8);
...
visit_typeParameterList(Ast_TypeParameterList* param_list)
{
  ...
  for (ListItem* li = list_first_item(&param_list->members);
        li != 0; li = li->next) {
    Ast_Name* name = list_item_get(li, Ast_Name*);
...
visit_headerTypeDeclaration(Ast_HeaderTypeDeclaration* header_decl)
{
  ...
  scope_push_decl(storage, &current_scope->decls, namedecl, NS_TYPE);
  hashmap_create(&header_decl->attr.fields, storage, HASHMAP_KEY_STRING, header_decl->attr.field_count+1);
...
visit_headerUnionDeclaration(Ast_HeaderUnionDeclaration* union_decl)
{
  ...
  scope_push_decl(storage, &current_scope->decls, namedecl, NS_TYPE);
  hashmap_create(&union_decl->attr.fields, storage, HASHMAP_KEY_STRING, union_decl->attr.field_count+1);
...
visit_structTypeDeclaration(Ast_StructTypeDeclaration* struct_decl)
{
  ...
  scope_push_decl(storage, &current_scope->decls, namedecl, NS_TYPE);
  hashmap_create(&struct_decl->attr.fields, storage, HASHMAP_KEY_STRING, struct_decl->attr.field_count+1);
...
visit_enumDeclaration(Ast_EnumDeclaration* enum_decl)
{
  ...
  scope_push_decl(storage, &current_scope->decls, namedecl, NS_TYPE);
  hashmap_create(&enum_decl->attr.fields, storage, HASHMAP_KEY_STRING, enum_decl->attr.field_count+1);
...
visit_errorDeclaration(Ast_ErrorDeclaration* error_decl)
{
  ...
  scope_push_decl(storage, &current_scope->decls, namedecl, NS_TYPE);
  hashmap_create(&error_decl->attr.fields, storage, HASHMAP_KEY_STRING, error_decl->attr.field_count+1);
...
visit_matchKindDeclaration(Ast_MatchKindDeclaration* match_decl)
{
  ...
  scope_push_decl(storage, &current_scope->decls, namedecl, NS_TYPE);
  hashmap_create(&match_decl->attr.fields, storage, HASHMAP_KEY_STRING, match_decl->attr.field_count+1);
...
visit_specifiedIdentifierList(Ast_SpecifiedIdentifierList* ident_list, Hashmap* decl_table)
{
  ...
  for (ListItem* li = list_first_item(&ident_list->members);
        li != 0; li = li->next) {
    visit_specifiedIdentifier(list_item_get(li, Ast_SpecifiedIdentifier*), decl_table);
  }
}
