...
visit_name(Ast_Name* name)
{
  ...
  hashmap_set(scope_map, HASHMAP_KEY_UINT64, (uint64_t)name, &current_scope);
}
...
visit_headerTypeDeclaration(Ast_HeaderTypeDeclaration* header_decl)
{
  ...
  hashmap_set(field_map, HASHMAP_KEY_UINT64, (uint64_t)header_decl, &field_scope);
...
visit_headerUnionDeclaration(Ast_HeaderUnionDeclaration* union_decl)
{
  ...
  Scope* field_scope = arena_malloc(storage, sizeof(*field_scope));
  hashmap_create(&field_scope->name_table, storage, HASHMAP_KEY_STRING, sizeof(NameEntry), 7, 511);
  hashmap_set(field_map, HASHMAP_KEY_UINT64, (uint64_t)union_decl, &field_scope);
...
visit_structTypeDeclaration(Ast_StructTypeDeclaration* struct_decl)
{
  ...
  Scope* field_scope = arena_malloc(storage, sizeof(*field_scope));
  hashmap_create(&field_scope->name_table, storage, HASHMAP_KEY_STRING, sizeof(NameEntry), 7, 511);
  hashmap_set(field_map, HASHMAP_KEY_UINT64, (uint64_t)struct_decl, &field_scope);
...
visit_structFieldList(Ast_StructFieldList* field_list, Scope* field_scope)
{
  ...
  for (Ast** p = list_cursor_begin(&field_list->members);
    ...
    visit_structField((Ast_StructField*)(*p), field_scope);
  }
}
...
visit_enumDeclaration(Ast_EnumDeclaration* enum_decl)
{
  ...
  Scope* field_scope = arena_malloc(storage, sizeof(*field_scope));
  hashmap_create(&field_scope->name_table, storage, HASHMAP_KEY_STRING, sizeof(NameEntry), 7, 511);
  hashmap_set(field_map, HASHMAP_KEY_UINT64, (uint64_t)enum_decl, &field_scope);
...
visit_errorDeclaration(Ast_ErrorDeclaration* error_decl)
{
  ...
  Scope* field_scope = arena_malloc(storage, sizeof(*field_scope));
  hashmap_create(&field_scope->name_table, storage, HASHMAP_KEY_STRING, sizeof(NameEntry), 7, 511);
  hashmap_set(field_map, HASHMAP_KEY_UINT64, (uint64_t)error_decl, &field_scope);
...
visit_matchKindDeclaration(Ast_MatchKindDeclaration* match_decl)
{
  ...
  Scope* field_scope = arena_malloc(storage, sizeof(*field_scope));
  hashmap_create(&field_scope->name_table, storage, HASHMAP_KEY_STRING, sizeof(NameEntry), 7, 511);
  hashmap_set(field_map, HASHMAP_KEY_UINT64, (uint64_t)match_decl, &field_scope);
...
visit_specifiedIdentifierList(Ast_SpecifiedIdentifierList* ident_list, Scope* field_scope)
{
  ...
  for (Ast** p = list_cursor_begin(&ident_list->members);
       p != 0; p = list_cursor_next(&ident_list->members)) {
    visit_specifiedIdentifier((Ast_SpecifiedIdentifier*)(*p), field_scope);
  }
}
...
visit_tableDeclaration(Ast_TableDeclaration* table_decl)
{
  ...
  hashmap_create(&prop_scope->name_table, storage, HASHMAP_KEY_STRING, sizeof(NameEntry), 7, 511);
  hashmap_set(field_map, HASHMAP_KEY_UINT64, (uint64_t)table_decl, &prop_scope);
...
visit_tablePropertyList(Ast_TablePropertyList* prop_list, Scope* prop_scope)
{
  ...
  for (Ast** p = list_cursor_begin(&prop_list->members);
    ...
    visit_tableProperty((Ast_TableProperty*)(*p), prop_scope);
  }
}
...
visit_simpleProperty(Ast_SimpleProperty* simple_prop, Scope* prop_scope)
{
  ...
  scope_push_decl(prop_scope, namedecl, NS_VAR);
...
pass_name_decl(ParsedProgram* p4program, Arena* _storage)
{
  ...
  hashmap_create(scope_map, storage, HASHMAP_KEY_UINT64, sizeof(Scope*), 7, 1023);
  field_map = &pass_result.field_map;
  hashmap_create(field_map, storage, HASHMAP_KEY_UINT64, sizeof(Scope*), 7, 1023);
