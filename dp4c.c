...
syn_struct_field()
{
  ...
  if (token_at->klass == TOK_IDENT)
  {
    ...
    if (id_info && id_info->scope_level >= scope_level)
      error("at line %d: selector '%s' has been previously declared", line_nr, result->name);
    result->selector = sym_add_selector(result->name);
    next_token();
    if (token_at->klass == TOK_SEMICOLON)
      next_token();
    else
      error("';' expected at line %d, got '%s'", line_nr, token_at->lexeme);
  }
  else
    error("identifier expected at line %d, got '%s'", line_nr, token_at->lexeme);
  return result;
}
  ...
  if (token_at->klass == TOK_IDENT)
  {
    ...
    if (token_at->klass == TOK_BRACE_OPEN)
    {
      ...
      if (token_at->klass == TOK_BRACE_CLOSE)
        ...
        error("at line %d: unknown type '%s'", line_nr, token_at->lexeme);
      else
        error("'}' expected at line %d, got '%s'", line_nr, token_at->lexeme);
      scope_pop_level();
    }
    ...
    error("at line %d: type '%s' has been previously declared", line_nr, token_at->lexeme);
  else
    error("identifier expected at line %d, got '%s'", line_nr, token_at->lexeme);
  return result;
}
...
syn_error_code()
{
  ...
  if (id_info && id_info->scope_level >= scope_level)
    error("at line %d: selector '%s' has been previously declared", line_nr, id->name);
    ...
    if (token_at->klass == TOK_IDENT)
    {
      ...
      while (token_at->klass == TOK_COMMA)
      {
        ...
        if (token_at->klass == TOK_IDENT)
        {
          ...
          error("missing parameter at line %d", line_nr);
        else
          error("identifier expected at line %d, got '%s'", line_nr, token_at->lexeme);
      }
  ...
  if (token_at->klass == TOK_TYPE_IDENT)
  {
    ...
    while (token_at->klass == TOK_COMMA)
    {
      ...
      else if (token_at->klass == TOK_INTEGER)
      {
        ...
        error("type identifier expected at line %d, got '%s'", line_nr, token_at->lexeme);
    }
  }
  else if (token_at->klass == TOK_INTEGER)
  {
    ...
    if (priority >= priority_threshold)
    {
      ...
      if (op_is_binary(op))
      {
        ...
        if (token_is_expression(token_at))
          ...
          error("expression term expected at line %d, got '%s'", line_nr, token_at->lexeme);
        ...
        if (token_is_expression(token_at))
        {
          ...
          while (token_at->klass == TOK_COMMA)
          {
            ...
            if (token_is_expression(token_at))
            {
              ...
              error("at line %d: expected an expression term, got '%s'", line_nr, token_at->lexeme);
          }
        }
        if (token_at->klass == TOK_PARENTH_CLOSE)
          next_token();
        else
          error("at line %d: '}' expected, got '%s'", line_nr, token_at->lexeme);
...
syn_transition_stmt()
{
  ...
  if (token_at->klass == TOK_IDENT)
    ...
    error("transition stmt expected at line %d, got '%s'", line_nr, token_at->lexeme);
  return result;
}
  ...
  if (token_at->klass == TOK_IDENT)
  {
    ...
    if (token_at->klass == TOK_BRACE_OPEN)
    {
      ...
      if (token_at->klass == TOK_KW_TRANSITION)
        ...
        error("'transition' expected at line %d, got '%s'", line_nr, token_at->lexeme);
      if (token_at->klass == TOK_BRACE_CLOSE)
        next_token();
      else
        error("'}' expected at line %d, got '%s'", line_nr, token_at->lexeme);
...
syn_parser_decl()
{
  ...
  if (token_at->klass == TOK_BRACE_OPEN)
  {
    ...
    if (token_at->klass == TOK_KW_STATE)
    {
      ...
      error("'state' expected at line %d, got '%s'", line_nr, token_at->lexeme);
    if (token_at->klass == TOK_BRACE_CLOSE)
      next_token();
    else
      error("'}' expected at line %d, got '%s'", line_nr, token_at->lexeme);
    ...
    error("'{' or ';' expected at line %d, got '%s'", line_nr, token_at->lexeme);
  sym_add_error_kw();
  return result;
}
...
syn_block_statement()
{
  ...
  if (token_at->klass == TOK_BRACE_CLOSE)
    ...
    error("block stmt expected at line %d, got '%s'", line_nr, token_at->lexeme);
  return result;
}
...
syn_p4program()
{
  ...
  if (token_is_declaration(token_at))
  {
    ...
    if (token_at->klass != TOK_EOI)
      error("expected end of input at line %d, got '%s'", line_nr, token_at->lexeme);
  }
  else
    error("expected a declaration at line %d, got '%s'", line_nr, token_at->lexeme);
