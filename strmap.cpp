...
Strmap* Strmap::create(Arena* storage, int segment_count)
{
  ...
  strmap = (Strmap*)storage->malloc(sizeof(Strmap) + sizeof(StrmapEntry**) * segment_count);
...
void Strmap::init(Arena* storage, int segment_count)
{
  ...
  entries.segments[0] = storage->malloc(sizeof(StrmapEntry*) * 16);
...
void Strmap::grow()
{
  ...
  entries.segments[last_segment] = storage->malloc(sizeof(StrmapEntry*) * segment_capacity);
  capacity = 16 * ((1 << (last_segment + 1)) - 1);
  for (int i = 0; i <= last_segment; i++) {
    segment_capacity = 16 * (1 << i);
    for (int j = 0; j < segment_capacity; j ++) {
      segment = (StrmapEntry**)entries.segments[i];
      segment[j] = 0;
    }
  }
...
StrmapEntry* Strmap::insert(char* key, void* value, bool return_if_found)
{
  ...
  entry = (StrmapEntry*)storage->malloc(sizeof(StrmapEntry));
  entry->key = key;
  entry->value = value;
