...
void
Debug_potential_types(Ast* p4program, Set* potential_types_)
{
...
visit_name(Ast* name)
{
  ...
  P = set_lookup_value(potential_types, name, 0);
  if (P) {
    printf("0x%x %s %s ... %d:%d\n", name, Debug_AstEnum_to_string(name->kind), name->name.strname,
           name->line_no, name->column_no);
    if (P->root) {
      Debug_print_potential_types(P);
    } else {
      printf("  (none)\n");
    }
  }
